...
PDF_PATH = ROOT / "output/pdf/matths-networking-poster-a3-v3.pdf"
...
text("교육비의 새로운 방향", 96, 844, 25, "bold")
text("보상이 다시 학습으로 이어지는 구조", 706, 844, 27, "bold")

# Replace the screen with an editorial payer / achiever / recipient manifesto.
# Preserve ample whitespace and make the recipient reversal the cyan punchline.
text("결제는", 96, 970, 37, color=MUTED)
text("부모가.", 96, 1060, 76, "heavy", tracking=-1.5, max_width=462)
line(96, 1136, 558, 1136)

text("성과는", 96, 1204, 37, color=MUTED)
text("학생이.", 96, 1300, 76, "heavy", tracking=-1.5, max_width=462)
line(96, 1374, 558, 1374)

text("보상도", 96, 1450, 37, color=CYAN)
text("학생에게.", 96, 1547, 76, "heavy", color=CYAN, tracking=-1.5, max_width=462)

# The parent / student motivation gap is a positioning statement, not data.
text("부모는 미래의 성적을,", 96, 1792, 30, "bold", max_width=462)
text("학생은 오늘의 동기를.", 96, 1845, 30, "bold", max_width=462)
...
text("결제액 일부 또는 전액 페이백", 706, 1565, 28, "bold")
...
text("www.matths.kr", 968, 2110, 36, "bold", max_width=330)
...
for filename, content in [("matths-networking-poster-v3.svg", SVG), ("source/matths-networking-poster-editable-text.svg", SVG_TEXT)]:
...
print(json.dumps({"pdf": str(PDF_PATH), "svg": str(OUT/"matths-networking-poster-v3.svg"), "text_blocks": len(TEXT_AUDIT), "qr_url": QR_URL}, ensure_ascii=False))

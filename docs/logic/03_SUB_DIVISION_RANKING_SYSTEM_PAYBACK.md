...
| `lockedPaybackScoreDays` | 진행 중 Sub 경기에 예치된 페이백 점수 |
| `lockedLearningDays` | Main 경기에서만 사용하는 예치 학습일수. Sub 경기에는 사용하지 않음 |
...
- 경기 신청 순간 `arenaOneOnOneProblemTypes.js`의 Arena 전용 JS 생성기가 주관식 준킬러 5문항을 만든다. T1~T9 등급·단원 슬롯·검수·보정 정책의 단일 원본은 `arenaOneOnOneDifficultyPolicy.js`이며 상세 기준은 `02_GOAT_ARENA_COMMON_MATCH_RULES.md` 3절을 따른다. 실제 문항 원본은 최종 티어별 생성기 교체 전까지 배치고사 심화 유형을 파일 단위로 독립 복사한 임시 구성이고 배치고사 파일을 직접 import하지 않는다.
- 양측은 완전히 같은 문제를 풀고, 난이도는 항상 방어자 티어에 해당하는 T1~T9 등급으로 확정한다.
...
- 상대 배정 전에는 개인 후보를 공개하지 않고, 매치 성립 뒤에는 서비스 닉네임을 표시한다. 실명·학교·지역·연락처는 공개하지 않는다. 단 유저가 GOAT Arena에서 실명 쓰기를 설정에서 적용한다면 실명은 공개한다.
...
일반 쟁탈전 신청 시 Sub Division 전역 고정값인 `normalStakePaybackScore = 1`점을 다음과 같이 예치한다.
...
`availableLearningDays`와 `lockedLearningDays`는 Sub 경기 생성·정산으로 변경하지 않는다. 방어자는 경기 생성 시 페이백 점수를 예치하지 않으며, 승패에 따른 이전·반환·소각은 공식 정산 단계에서 한 번만 처리한다.

학습 가능 일수와 페이백 점수는 서로의 값에서 파생하거나 자동 동기화하지 않는다. Sub 경기 결과는 페이백 점수 장부만 변경한다.
...
Sub Division에는 별도 휴면 상태나 휴면 복귀 배치고사가 없다. 29일 학습권 패키지와 페이백 주기가 끝나면 기존 만료·재구매 규칙을 적용한다. Main 휴면 강등 당시 20일 차감 후 남은 학습일수는 Sub 이용 주기와 페이백 점수에 넣지 않고 별도로 보관한다. 강등 사용자는 일반 Sub Division의 새 패키지 활성화·배치고사·29일 학습·페이백 달성 절차를 모두 거쳐야 하며, Main Division에 다시 진입하는 순간에만 보관 일수를 새 Main 이용 주기에 추가한다.

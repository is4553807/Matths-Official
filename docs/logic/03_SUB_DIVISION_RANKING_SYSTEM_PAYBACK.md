...
- 경기 신청 순간 활성 DB 카탈로그가 선택한 공식 사고 유형과 그 유형에 직접 결속된 독립 검산 생성기가 주관식 5문항을 만든다. 공개 난이도는 Unranked `U1~U9`이며, 기존 `T1~T9`는 배포된 문제 유형 DB를 찾는 내부 호환 키로만 사용한다. 공통수학Ⅰ·Ⅱ·대수·확률과 통계·미적분Ⅰ 유형 골격은 `arenaOneOnOneTypeSkeletons.js`가 관리한다. 1~5번 모두 13·14·20·21·27·28번형 준킬러 사고 구조에서 배정하며 공개 난이도마다 최소 30개 수치·조건 변형 유형을 둔다. 상세 기준은 `02_GOAT_ARENA_COMMON_MATCH_RULES.md` 3절을 따른다.

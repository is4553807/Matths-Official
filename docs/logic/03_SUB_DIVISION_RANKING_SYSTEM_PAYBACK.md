...
- 경기 신청 순간 2016~2026 EBSi 고3 전월 모의고사 전 문항의 정답률 분류 근거와 검산 생성기가 주관식 5문항을 만든다. 공개 난이도는 Unranked 전용 `U1~U9`이며 기존 `T1~T9`는 DB 호환 키로만 사용한다. U1은 D1 4문항+D2 1문항으로 시작하고 U9은 D4 1문항+D5 2문항+D6 2문항까지 올라간다. Unranked에는 킬러 D8·D9를 넣지 않는다. 한 경기 안 유형·생성기 중복을 금지하고 양쪽 참가자의 최근 공식 경기 5개 유형을 우선 제외한다. 상세 기준은 `02_GOAT_ARENA_COMMON_MATCH_RULES.md` 3절을 따른다.

...
+ Unranked U1~U9·Ranked R1~R9의 독립 D1~D9 조합표에 따른 주관식 5문항 자동 생성·자동 검산
...
- `arenaMatchDifficultyPlan.js`와 `arenaOneOnOneDifficultyPolicy.js`가 Unranked `U1~U9`·Ranked `R1~R9`의 서로 분리된 D1~D9 조합, 방어자 앵커, 목표 정답률, 5슬롯 곡선, 단원 구성, 1~999 자연수 답과 실측 보정 임계값을 관리한다. 기존 `T1~T9`는 DB 카탈로그 호환용 내부 키이며 사용자·룰 페이지에는 노출하지 않는다. 현재 신규 경기는 2016~2026 전체 조사로 만든 Arena 전용 PDF 스켈레톤 풀을 사용하고 활성 DB 카탈로그는 호환·검증용으로 보존한다. R7~R9의 킬러 슬롯만 29·30번형 다중 조건 생성기를 사용한다. 한 경기 안에서는 공개 유형 ID·공식 기본 유형·실제 생성기 세 가지가 모두 중복되지 않아야 하고 참가자 양쪽의 최근 공식 경기 5개 유형을 먼저 제외한다. 자동 검산 실패 시 문제·경기·예치·원장을 만들지 않는다.

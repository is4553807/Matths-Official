...
모든 주기 자동 작업은 프로세스 메모리 플래그만으로 중복을 막지 않는다. 실행 직전 `schedulerLeases` 컬렉션의 작업별 MongoDB 분산 임대를 원자적으로 선점하고, 실행 중에는 만료 시각을 갱신한다. 다른 서버는 활성 임대가 있는 작업을 건너뛰고, 완료·실패 또는 임대 만료 뒤 다음 서버가 이어받는다. 경기 타이머, 학습권 차감·만료 알림, 주간 공식 모의고사 상태 전환, 시즌, 데이터 분석, 무결성 점검, outbox, 증거·아카이브 보존 삭제, R2 백업과 사용자 임시 파일 정리에 동일하게 적용한다.

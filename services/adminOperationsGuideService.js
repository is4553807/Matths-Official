...
const CATEGORY_DEFINITIONS = [
  ...
  },
  {
    ...
    pattern: /Concept|Problem|Assessment|Practice|LearningProgress|DailyPlan|Wrong|Coach/i,
  },
  {
    id: "community",
    label: "게시판·콘텐츠 운영",
...
const STORAGE_MATRIX = [
  {
    ...
    primary: "서버 영구 디스크 / storage/archive",
    backup: "Cloudflare R2 비공개 버킷",
    retention: "운영자 삭제 전까지. 휴지통 이동 후 30일",
    access: "로그인·폴더 권한 검사 후 Express가 전송",
  },
  {
    owner: "운영자",
    ...
    access: "상품·공개 시각·관리자 권한 검사 후 전송",
  },
  {
    owner: "사용자",
    fileType: "게시판 사진·첨부",
...
const SCHEDULERS = [
  ...
  ["아카이브 휴지통 삭제", "6시간", "30일 경과 자료의 로컬·R2·DB 영구 삭제"],
  ["운영자 파일 R2 백업", "업로드 약 10초 후 + 매일 03:30 KST", "SHA-256이 같은 객체는 건너뛰는 증분 백업"],
  ["R2 복원 점검", "관리자 수동 실행", "비공개 객체를 임시 경로로 내려받아 SHA-256을 검증하고, 로컬 원본 누락 파일만 원자적으로 복구"],
...
const INCIDENT_PLAYBOOK = [
  ...
  },
  {
    title: "운영자 디스크·R2 이상",
    checks: [
      "디스크 70% 주의·85% 경고·95% 업로드 차단 상태 확인",
      "ARCHIVE_STORAGE_DIR가 배포 플랫폼의 영구 볼륨인지 확인",
      "R2 backupStatus=FAILED 항목의 backupError 확인 후 수동 백업 실행",
      "복원 검증 전 로컬 원본과 R2 객체를 동시에 삭제하지 않기",
    ],
  },
  {
    title: "경기·랭킹 정합성 이상",
...
const OPERATING_WORKFLOWS = [
  ...
  },
  {
    ...
    hardStops: "storage/archive를 public 아래로 옮기거나 R2 공개 URL을 사용자에게 직접 제공하지 않습니다.",
  ...
  },
  {
    ...
    steps: [
      "T1~T9 원본 JSON을 가져올 때는 9개 티어·티어당 30개·고유 참고 문항 270개와 대수 12·미적분Ⅰ 12·확률과 통계 6 구성을 먼저 자동 검증합니다.",
      ...
      "각 참고 유형은 평가센터의 독립 검산 생성기와 소스 SHA-256으로 연결합니다. 경기 직전에도 생성기 승인 상태, 소스 일치, 계산기 불필요, 1~999 자연수 답을 다시 확인합니다.",
      "새 유형은 문제 데이터 화면의 ‘1대1 문제 유형 추가’에서 이름, 승인 생성기 기준 유형과 T1~T9 배정 범위를 선택합니다. 서버는 기준 생성기를 3회 다시 검산하고 기존 30문항 구성을 유지한 새 카탈로그 버전을 활성화합니다.",
      ...
      "T1~T9마다 사용 상태인 서로 다른 유형을 최소 5개 선택합니다. 화면에 표시되는 유형은 서버에 등록되고 3자리 이하 자연수 답 조건을 통과할 수 있는 생성기뿐입니다.",
...
const ENVIRONMENT_CONFIGURATION = [
  ...
  ["운영자 원본", "ARCHIVE_STORAGE_DIR, LOCAL_STORAGE_PERSISTENT", "아카이브·주간 모의고사", "public 밖 영구 볼륨, 운영에서 값 1"],
  ["재해 복구", "R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET", "운영자 원본 증분 백업", "비공개 버킷·Object Read & Write"],

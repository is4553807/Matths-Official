    ...
    rules: [
      {
        ...
        title: "참가 자격과 경기 종류",
        sections: [
          { title: "이용 자격", body: ["학습권 패키지가 활성 상태이고 배치고사를 완료해야 합니다.", "사용 가능한 정기권 학습 가능 일수가 0일이 되더라도 마지막 공식 공격 제출 마감 전까지는 Unranked 공식 경기를 허용하며, 마감 뒤 공격·방어·복수전과 신규 매칭이 모두 중단됩니다.", "학습권 만료 뒤 29일 학습권 패키지를 다시 구매하면 이전 배치를 재사용하지 않고 배치고사를 다시 완료해야 합니다."] },
      ...
      },
      {
        ...
        title: "도전 대상과 자동 매칭",
        sections: [
          { title: "상위 순위 우선", body: ["모든 티어에서 서버가 먼저 공격자와 같은 티어이면서 공격자보다 티어 내부 순위가 높은 적격 사용자를 찾습니다.", "같은 티어의 상위 후보가 한 명이라도 있으면 바로 위 티어 후보로 넘어가지 않습니다.", "방어전 정산 후 1시간이 지나지 않은 사용자는 다른 후보가 있을 때 후순위로 배치합니다. 모든 후보가 1시간 이내 정산 상태라면 매칭을 막지 않고 기존 공정 순서로 선정합니다.", "후순위 조정 뒤 후보군에서는 최근 72시간 일반 쟁탈전 방어 횟수가 가장 적은 사용자를 먼저 모으고, 동률이면 마지막 방어 배정이 가장 오래된 사용자를 우선합니다. 그 시각까지 같을 때만 무작위로 한 명을 정합니다.", "복수전은 72시간 방어 횟수와 마지막 방어 배정 집계에서 제외하며, 상대의 닉네임이나 특정 계정은 직접 고를 수 없습니다."] },
          ...
          { title: "일일 공격·방어 슬롯", body: ["한 사용자는 동시에 정산되지 않은 공식 경기 하나만 가질 수 있으며, 경기가 정산되면 현재 티어의 남은 방어 슬롯으로 다음 방어전에 배정될 수 있습니다.", "도전자로 승리하면 그날 남은 일반 공격만 잠기고 방어 자격은 현재 티어의 일일 방어 상한까지 유지됩니다.", "Unranked에는 경기 종료 뒤 6시간 하드 쿨다운을 적용하지 않습니다. 정산 뒤 1시간 후순위는 다른 후보가 없을 때 매칭을 막지 않는 소프트 조정입니다."] },
      ...
      },
      {
        ...
        sections: [
          ...
          { title: "도전자 승리", body: ["시작 티어와 관계없이 도전자가 이기면 예치한 페이백 점수 1점을 전부 반환합니다.", "티어·티어 내 순위·GP는 교환되며 페이백 점수의 순변화는 없습니다."] },
          { title: "방어자 승리", body: ["방어자의 현재 Arena 상태를 유지하고 도전자가 예치한 페이백 점수 1점을 가져옵니다."] },
          { title: "0점과 패키지 만료", body: ["페이백 점수가 0점이면 일반 공격과 복수전 신청만 중단되며, 활성 29일 패키지의 남은 기능과 방어 자격은 유지됩니다.", "정기권 학습 가능 일수가 0일이 되더라도 마지막 공식 공격 제출 마감 전까지는 해당 공격을 허용하며, 마감 뒤 GOAT Arena 전체가 잠기고 방어 후보에서도 제외됩니다."] },
        ],
      },
      {
        number: 4,
        ...
        sections: [
          ...
          { title: "정상 완료", body: ["복수전 신청자가 이기면 Arena 상태를 다시 교환하고 예치한 2점 중 1점을 반환하며, 나머지 1점은 수수료로 소각합니다.", "복수전 신청자가 지면 Arena 상태를 유지하고 예치한 2점 중 1점은 방어자에게 이전하며, 나머지 1점은 수수료로 소각합니다."] },
    ...
    rules: [
      ...
      },
      {
        ...
        sections: [
          ...
          { title: "정상 완료", body: ["Unranked 복수전과 같은 구조로 신청 금액에서 1일 수수료를 먼저 소각합니다.", "공격자가 이기면 Arena 상태를 교환하고 2×S-1일을 공격자에게 반환하며 1일을 수수료로 소각합니다.", "방어자가 이기면 Arena 상태를 유지하고 2×S-1일을 방어자에게 이전하며 1일을 수수료로 소각합니다."] },
